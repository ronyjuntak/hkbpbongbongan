  ...
  <div id="loading-indicator">
    ...
    <header class="hero">
       <H1><span>Bongbongan</span></H1>
        <p>Status Pendaftaran Keluarga</p>
      ...
      <div class="judul_1" id="targetElement">
        <div class="">
          <table table class="aesthetic-table-user" >
            <thead>
              <tr>
                <th  style=" font-weight:40px ;border: none; background-color: #37545f ; color:white; padding:40px;  "colspan="3">Status Pendaftaran Keluarga</th>
              </tr>
              <tr>
                <th class="judul-tabel">Nama Keluarga</th>
                <th class="judul-tabel">Status</th>
              </tr>
            </thead>
            @foreach($userKeluargas as $data)
            @if($data->status == 0)
            <tbody class="info-3">
              <tr>
                <td class="lebar">{{$data->nama_keluarga}}</td>
                <td>
                  @if($data->status == 0)
                    <label style="background-color:darkorange" for="">Menunggu</label>
                  @elseif($data->status == 1)
                    <label style="background-color:green" for="">Disetujui</label>
                  @elseif($data->status == 12)
                    <label style="background-color:red" for="">Ditolak</label>
                  @endif
                </td>
              </tr>
            </tbody>
